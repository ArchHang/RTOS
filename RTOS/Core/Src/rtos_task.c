...
/**
 * @brief 栈溢出检测(方法2: 校验栈底魔数 + 检查栈指针越界)。
 */
static void check_stack_overflow(rtos_tcb_t *tcb)
{
    ...
    /* 检查栈指针是否越界(低于栈底) */
    if ((uint32_t)tcb->top_of_stack < (uint32_t)tcb->stack_base) {
        rtos_stack_overflow_hook(tcb);
    }
#else
    (void)tcb;
                                      ...
                                      const char *name)
{
    ...
    /* 最小栈 = rtos_port_init_stack 伪帧 18 字(硬件帧 8 + R4-R11 8 +
     * EXC_RETURN 1 + 对齐填充 1) + 8 字节对齐最多损耗 1 字 + 裕量 1 字。
     * 低于此值时 sp 会下溢到栈缓冲之前, 越界写相邻内存。 */
    if (stack_size < 20U) {
        return RTOS_ERR_PARAM;
    }
                               ...
                               void *arg, rtos_prio_t priority, const char *name)
{
    ...
    /* 与 rtos_task_create_static 相同的最小栈校验(见上文注释) */
    if (stack_size < 20U) {
        return RTOS_ERR_PARAM;
    }
...
rtos_status_t rtos_task_suspend(rtos_tcb_t *tcb)
{
    ...
    if (tcb->state == RTOS_TASK_READY || tcb->state == RTOS_TASK_RUNNING) {
        ...
        /* 从延时表移除(复用 sched_unblock 的延时表清理逻辑前半部分) */
        rtos_tcb_t *p = NULL;
        rtos_tcb_t *c = rtos_kernel.delay_head;
        while (c != NULL) {
            if (c == tcb) {
                break;
            }
        ...
        if (c == tcb) {
            if (p == NULL) {
                rtos_kernel.delay_head = c->next_ready;
            } else {
                p->next_ready = c->next_ready;
            }
            tcb->next_ready = NULL;
        }
    }
...
void rtos_idle_task(void *arg)
{
    ...
    for (;;) {
        ...
        if ((idle_count & 0xFFU) == 0U) {
            RTOS_PORT_ENTER_CRITICAL();
            /* 检查就绪表中所有任务 */
            for (rtos_prio_t p = 0; p < RTOS_CONFIG_MAX_PRIORITIES; p++) {
                rtos_tcb_t *head = rtos_kernel.ready_heads[p];
                if (head != NULL) {
                    rtos_tcb_t *tcb = head;
                    do {
                        check_stack_overflow(tcb);
                        tcb = tcb->next_ready;
                    } while (tcb != head);
                }
            }
            /* 检查延时表中的任务(单链表, next_ready 串接) */
